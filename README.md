...
Run the `noVNCAutoInstallation.sh` bash script included in this repo. Credits for the script to <a href='https://github.com/MitchRatquest'>MitchRatquest</a> and to <a href='https://github.com/novnc/noVNC'>noVNC</a> for their utility. Here I am running `htop` and `wavemon` with a `screen` session. Add `@reboot bash /home/stuxnet/samba/noVNC/vnc.sh` to `crontab -e` to start noVNC on startup and ditch serial connection and/or PuTTY all together.
<br>
