...
Setup a cronjob in `crontab -e` to run the py.py script on startup or reboot. It will always try to connect as client, using any/all credentials supplied in `hostapd` config. If after 3 failed attempts, over 30 seconds, the device fails to ping a specified remote server (www.google.com) then the device will run FriendlyArm's binary to turn on Access Point mode. From there, you can connect via phone, browser, etc... and tinker. If you want it to turn back into a client and connect to home/work/etc... SSID, you will need to do so manually by running `turn-wifi-into-apmode no`
